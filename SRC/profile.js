...
export async function handleGetMyProfile(request, env) {
  ...
  if (!profile) {
    await env.DB.prepare(
      `INSERT INTO user_profiles (user_id, display_name, bio, avatar_url, profile_emoji, bg_color, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    )
...
export async function handleUpdateProfile(request, env) {
  ...
  if (profile_emoji !== undefined) {
    ...
    if (profile_emoji && !allowedEmojis.includes(profile_emoji)) {
  ...
  if (bg_color !== undefined) {
    const color = (bg_color && /^#[0-9A-Fa-f]{6}$/.test(bg_color)) ? bg_color : '#1a1a2e';
