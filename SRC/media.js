...
export async function handleUpload(request, env) {
  ...
  if (approxBytes > limit) return json({ error: 'too_large', detail: 'Файл ' + approxBytes + ' байт превышает лимит ' + limit + ' байт' }, 413);
  ...
  await env.DB.prepare(
    'INSERT INTO media (id, user_id, type, shard_id, blob_key, caption, size_bytes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
...
export async function handleFeed(request, env) {
  ...
  let query = 'SELECT media.*, users.name as author_name, users.avatar_url as author_avatar FROM media JOIN users ON users.id = media.user_id WHERE media.created_at < ? AND media.deleted_at IS NULL';
  const binds = [cursor];
  if (userId) { query += ' AND media.user_id = ?'; binds.push(userId); }
...
export async function handleSaved(request, env) {
  ...
  const { results } = await env.DB.prepare(
    'SELECT media.*, users.name as author_name, users.avatar_url as author_avatar FROM saves JOIN media ON media.id = saves.media_id JOIN users ON users.id = media.user_id WHERE saves.user_id = ? AND media.deleted_at IS NULL ORDER BY saves.created_at DESC LIMIT 50'
  ).bind(user.id).all();
...
export async function handleDeleteMedia(request, env, mediaId) {
  ...
  await env.DB.prepare('UPDATE media SET deleted_at = ? WHERE id = ?').bind(Date.now(), mediaId).run();
