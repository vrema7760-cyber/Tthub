...
export async function handleMediaContent(request, env, mediaId) {
  ...
  if (!shard) return json({ error: 'shard_not_found' }, 404);
  
  const blobResult = await fetchBlobFromShard(env, shard, media.blob_key);
  if (!blobResult) return json({ error: 'blob_missing' }, 404);
  
  // ✅ Важно: возвращаем Response с правильными заголовками
  return new Response(blobResult.body, {
    headers: {
      'Content-Type': media.mime_type || (media.type === 'video' ? 'video/mp4' : 'image/jpeg'),
      'Cache-Control': 'public, max-age=31536000',
      'Content-Length': blobResult.headers?.get('Content-Length') || ''
    }
  });
}
