...
body { min-height: 100vh; position: relative; padding-bottom: 80px; }
.aurora-bg {
  position: fixed; inset: 0; z-index: -2; overflow: hidden; background: var(--bg-primary);
}
.aurora-blob {
...
.aurora-blob:nth-child(2) {
  width: 350px; height: 350px; background: var(--accent-orange);
  top: 40%; right: -100px; animation-delay: -7s;
}
...
.bottom-nav {
  ...
  padding: 12px 20px; background: rgba(10, 10, 26, 0.95);
...
.nav-item {
  ...
  padding: 8px; color: var(--text-secondary); text-decoration: none;
  font-size: 11px; transition: all 0.2s; border: none; background: none;
}
...
.media-card:hover {
  transform: translateY(-4px); border-color: var(--accent-purple); box-shadow: var(--shadow);
}
...
.media-type-badge {
  ...
  background: rgba(0, 0, 0, 0.7); border-radius: 20px; font-size: 12px;
...
.media-caption { font-size: 13px; color: var(--text-secondary); line-height: 1.4; margin-bottom: 10px; }
.media-actions { display: flex; gap: 16px; font-size: 13px; color: var(--text-secondary); }
.media-action {
  display: flex; align-items: center; gap: 4px; background: none; border: none;
  color: inherit; padding: 4px 8px; border-radius: 8px; transition: all 0.2s;
}
...
.live-badge {
  ...
  font-size: 13px; font-weight: 600; animation: pulse 2s infinite;
}
...
.stream-card {
  ...
  border: 1px solid var(--border); border-radius: 16px; overflow: hidden; transition: all 0.3s;
}
...
.chat-avatar { width: 48px; height: 48px; border-radius: 50%; object-fit: cover; border: 2px solid var(--accent-purple); }
.chat-info { flex: 1; }
...
.chat-header {
  ...
  display: flex; align-items: center; gap: 12px; background: rgba(21, 21, 40, 0.9);
}
.chat-messages {
...
.chat-input-area {
  ...
  display: flex; gap: 8px; background: rgba(21, 21, 40, 0.9);
}
.chat-input {
...
.profile-bio { font-size: 14px; color: var(--text-secondary); text-align: center; max-width: 400px; line-height: 1.5; }
...
@media (max-width: 240px) {
  ...
  .media-card, .chat-item, .profile-header, .modal {
    background: white; border: 2px solid black; border-radius: 0;
  }
...
<script>
const state = { user: null, currentSection: 'feed', currentChat: null, chatPollInterval: null, easterEggClicks: 0 };
...
function switchSection(sectionName) {
  ...
  if (sectionName !== 'chats' && state.chatPollInterval) { clearInterval(state.chatPollInterval); state.chatPollInterval = null; }
...
animateParticles();
document.addEventListener('pointerdown', (e) => { for (let i = 0; i < 8; i++) createParticle(e.clientX, e.clientY); });
...
document.addEventListener('mouseover', (e) => {
  if (e.target.matches('button, a, .media-card, .chat-item, .icon-btn')) cursor.classList.add('active');
});
document.addEventListener('mouseout', (e) => {
  if (e.target.matches('button, a, .media-card, .chat-item, .icon-btn')) cursor.classList.remove('active');
});
...
if (window.location.hash) {
  ...
  if (['feed', 'streams', 'chats', 'profile'].includes(section)) switchSection(section);
}
</script>
</body>
</html>`;
